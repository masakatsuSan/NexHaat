...
export const createLot = async (req, res) => {
  try {
    ...
    if (!crop || !quantity || !quality || !location) return res.status(400).json({ message: "Crop, quantity, quality, and location are required." });
    ...
    const id = uuidv4();
    const lot = new FarmerLot({ _id: id, farmer_id: req.user.id, commodity: crop, variety: location, quantity_quintals: qtyNum, quality_grade: quality, harvest_date: harvest_date || null, expected_price: priceNum, status: "LISTED" });
    await lot.save();
    res.status(201).json({ success: true, lot: { id, crop, quantity, quality, location, status: "LISTED" } });
...
export const getMyLots = async (req, res) => {
  const lots = await FarmerLot.find({ farmer_id: req.user.id })
    .sort({ created_at: -1 })
    .select("_id commodity variety quantity_quintals quality_grade expected_price harvest_date status created_at")
    .lean();
  const mapped = lots.map(lot => ({
    id: lot._id,
    crop: lot.commodity,
    location: lot.variety,
